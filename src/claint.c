...
{
    char *integer = (char *)malloc((MAXLEN + 1) * sizeof(char)); // +1 for '\0'
    memset(integer, '0', (MAXLEN + 1) * sizeof(char));
    integer[MAXLEN] = '\0';
    return integer;
}
...
int compareCLAINTS(char *claint_a, char *claint_b)
{
    ...
    {
        if (claint_a[i] == claint_b[i])
            i++;
        else if (claint_a[i] > claint_b[i])
            return 1;
        else
            return -1;
    }
    if (claint_a[i] == '\0' && claint_b[i] != '\0')
        return -1;
    if (claint_a[i] != '\0' && claint_b[i] == '\0')
        return 1;
    return 0;
